...
public class ApiGatewayConfiguration {
	...
	RouteLocator getwayRoute(RouteLocatorBuilder routeLocatorBuilder) {
		 ...
		 */
		routeLocator = routeLocator.route(p -> p.path("/currency-exchange/**").uri("lb://d2-zipkin-currency-exchange-service"))
				.route(p -> p.path("/currency-conversion-feign/**")
						.uri("lb://d3-zepkin-currency-conversion-service-openfeign"));
